-- Update the evaluate_ticket_approval_rules function to handle Yes/No dropdown for LME Action Needed
...
begin
  ...
  if p_pricing_type in ('Formula', 'Index') then
    -- For B2B Formula deals, only route to Hedging if LME Action Needed is "Yes"
    if p_pricing_type = 'Formula' and p_transaction_type = 'B2B' then
      if p_lme_action_needed = 'Yes' then
