-- Update evaluate_ticket_approval_rules function to handle integer lme_action_needed (1 = Yes, 0 = No)
CREATE OR REPLACE FUNCTION public.evaluate_ticket_approval_rules(
  ...
  p_lme_action_needed integer DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
...
begin
  ...
  if p_pricing_type in ('Formula', 'Index') then
    -- For B2B Formula deals, only route to Hedging if LME Action Needed = 1 (Yes)
    if p_pricing_type = 'Formula' and p_transaction_type = 'B2B' then
      if p_lme_action_needed = 1 then
