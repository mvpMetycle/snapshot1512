-- Update the evaluate_ticket_approval_rules function to handle B2B Formula deals with LME action needed check
CREATE OR REPLACE FUNCTION public.evaluate_ticket_approval_rules(
  ...
  p_lme_action_needed text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
...
begin
  ...
  -- Rule 2: Deals requiring hedge (modified for B2B Formula deals)
  if p_pricing_type in ('Formula', 'Index') then
    -- For B2B Formula deals, only route to Hedging if LME Action Needed is checked
    if p_pricing_type = 'Formula' and p_transaction_type = 'B2B' then
      if p_lme_action_needed is not null and p_lme_action_needed != '' then
      ...
      -- For all other Formula deals (non-B2B) and all Index deals, always require approval
      if not ('Hedging'::approver_role = any(v_required_approvers)) then
        v_required_approvers := array_append(v_required_approvers, 'Hedging'::approver_role);
      end if;
      if not ('CFO'::approver_role = any(v_required_approvers)) then
